...
extension PetitionTableViewController : UITableViewDelegate, UITableViewDataSource, UIScrollViewDelegate{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.commentLabel.text = "청원일 : " + post.propose_dt
        cell.dangerView.text = "청원인 : " + "\(post.proposer)"
    ...
    func showPostDetail(post: PetitionPost){
        let detailViewController = PetitionDetailViewController(post: post)
        navigationController?.pushViewController(detailViewController, animated: true)
    }
    //청원 계류현황
    func fetchPosts(page: Int, completion: @escaping ([PetitionPost]?, Error?) -> Void) {
                    ...
                    for item in row {
                        if let billName = item["BILL_NAME"] as? String,
                           ...
                           let approver = item["APPROVER"] as? String,
                            ...
                            let post = PetitionPost(bill_name: billName, approver: approver, proposer: proposer, committee: committee, bill_no: billNo, propose_dt: propose_dt)
                            posts.append(post)
                        }
                    }
