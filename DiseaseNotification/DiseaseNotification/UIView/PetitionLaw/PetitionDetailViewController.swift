...
class PetitionDetailViewController : UIViewController {
    let post : PetitionPost
    //이니셜라이저를 사용하여 Post 객체를 전달받아 post 속성에 저장
    init(post: PetitionPost) {
        self.post = post
    ...
    func setupViews() {
        ...
        hospitalBtn.setTitle("  국민동의청원으로 이동", for: .normal)
    ...
    @objc func PetitionBtnTapped() {
        ...
        self.navigationController?.pushViewController(PetitionWebViewController(post: post.bill_name), animated: true)
    }
}
